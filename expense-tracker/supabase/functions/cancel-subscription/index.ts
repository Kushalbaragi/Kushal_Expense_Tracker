...
Deno.serve(async req => {
  ...
  // Try to let the user keep access through the current cycle. Razorpay rejects
  // cancel_at_cycle_end when no cycle has actually started yet (e.g. the customer
  // closed Checkout before authenticating) — fall back to immediate cancellation,
  // which is harmless there since nothing was ever charged.
  let { res: rpRes, data: rpData } = await razorpayCancel(1)
